...
def main():
    """Build the LightGBM-ready feature table from the cleaned dataset."""
    input_path = PROJECT_DIR / "data" / "interim" / "lendingclub_clean.csv"
    df = pd.read_csv(input_path)

    df["term"] = df["term"].astype(str).str.extract(r"(\d+)")[0].astype(float)

    # Convert percentage-like columns without hard-coding every source dtype.
    ...
    # Convert dates into model-friendly month offsets.
    date_cols = ["issue_d", "last_pymnt_d", "earliest_cr_line", "sec_app_earliest_cr_line"]
    ...
    df.drop(columns=[col for col in ["earliest_cr_line", "sec_app_earliest_cr_line"] if col in df], inplace=True)
    ...
    # Keep the declared feature set, filling only categorical missing values.
    for col in features:
        if col in df.columns and df[col].isna().any() and df[col].dtype == "object":
            df[col] = df[col].fillna("missing")

    output_path = PROJECT_DIR / "data" / "processed" / "lendingclub_features_for_lightgbm.csv"
