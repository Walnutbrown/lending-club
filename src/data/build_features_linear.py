...
def main():
    ...
    # 퍼센트 기호는 없지만 숫자로 변환해야 하는 퍼센트 의미 컬럼 처리
    ...
    for col in features:
        ...
        if df[col].isnull().sum() > 0:
            if df[col].dtype == 'object':
                ...
                df[f'{col}__nanflag'] = df[col].isnull().astype(int)
                df[col] = df[col].fillna(0)
                df[f'{col}__nan_x_value'] = df[col] * df[f'{col}__nanflag']
            else:
                pass  # datetime 등은 스킵
    
    # 3) 저장
    Path('01_data/01-3_processed/').mkdir(parents=True, exist_ok=True)
    df.to_csv('../../01_data/01-3_processed/lendingclub_features_for_linear.csv', index=False)
    print('✅ 파일 저장 완료')
